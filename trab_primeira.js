var prompt = require('prompt-sync')()

var LocalStorage = require('node-localstorage').LocalStorage;
localStorage = new LocalStorage('./dados');

while(true){
    titulo('Controle Pessoal de Filmes')
    console.log('1. Incluir Filmes')
    console.log('2. Listar Filmes')
    console.log('3. Pesquisar por Gênero')
    console.log('4. Excluir Filmes')
    console.log('5. Estastísticas')
    console.log('6. Finalizar')
    var opcao = Number(prompt('Opção: '))
    if (opcao == 1){
        incluir()
    }else if (opcao == 2){
        listar()
    }else if (opcao == 3){
        pesquisar()
    }else if (opcao == 4){
        excluir()
    }else if (opcao == 5){
        estatisticas()
    }else{
        break
    }

}

function titulo(mensa){
    console.log()
    console.log(mensa)
    console.log('================================')
}

function incluir(){
    titulo('Inclusão de Filmes: ')
    var nome = prompt('Título do filmes: ')
    var genero = prompt('Gênero: ')
    var duracao = prompt('Duração (min): ')
    var nota = prompt('Nota (1 até 5): ')

    var lista = ''
    //Se existe um arquivo com o nome do filmes.txt
    if (localStorage.getItem('filmes.txt')){
        //Atribui os filmes na ordem com uma quebra de linha
        lista = localStorage.getItem('filmes.txt') + '\n'
    }

    localStorage.setItem('filmes.txt', lista + nome+';'+genero+';'+duracao+';'+nota)

    console.log(`FILME CADASTRADO COM SUCESSO`)

}

function listar() {
    titulo('Listagem de Filmes')

    //Se não existe um arquivo com o nome do filmes.txt
    if (! localStorage.getItem('filmes.txt')){
       console.log('OBS:..Não há filmes cadastrados!')
       return
    }

    // lê o conteúdo de filmes.txt e atribui para lista
    var lista = localStorage.getItem('filmes.txt')

    //Separa o conteúdo de lista em elementos de vetor a cada ocorrência
    var linhas = lista.split('\n')

    console.log(`N: Tútilo do Filme..............: Gênero...............: Duração Nota`)

    var num = 0

    //Obtém cada linha do vetor linhas e atribui para linha
    for(linha of linhas){
        //separa cada linha em elementos de vetor pela ocorrência do ';'
        var partes = linha.split(';')

        num++

        var nome = partes[0]
        var genero = partes[1]
        var duracao = partes[2]
        var nota = partes[3]
        
        //converter número para uma string
        console.log(`${String(num).padStart(3,' ')} ${nome.padEnd(30,' ')} ${genero.padEnd(20,' ')} ${duracao.padStart(5,' ')} ${nota.padStart(5,' ')}`)
    }

}

function pesquisar(){
    titulo('Pesquisa de Filmes por Gênero')

    //Se não existe um arquivo com o nome do filmes.txt
    if (! localStorage.getItem('filmes.txt')){
       console.log('OBS:..Não há filmes cadastrados!')
       return
    }

    var pesq = prompt('Gênero: ')

    // lê o conteúdo de filmes.txt e atribui para lista
    var lista = localStorage.getItem('filmes.txt')

    //Separa o conteúdo de lista em elementos de vetor a cada ocorrência
    var linhas = lista.split('\n')

    //Para Controlar se há filmes do gênero informado
    var contador = 0

    console.log(`\n Tútilo do Filme..............: Gênero...............: Duração Nota`)

    //Obtém cada linha do vetor linhas e atribui para linha
    for(linha of linhas){
        //separa cada linha em elementos de vetor pela ocorrência do ';'
        var partes = linha.split(';')

        var nome = partes[0]
        var genero = partes[1]
        var duracao = partes[2]
        var nota = partes[3]

        if(genero.toUpperCase() == pesq.toUpperCase()){        
            console.log(`${nome.padEnd(30,' ')} ${genero.padEnd(20,' ')} ${duracao.padStart(5,' ')} ${nota.padStart(5,' ')}`)
            contador++
        }
    }

    if(contador == 0){
        console.log('Obs: Não há filmes do gênero informado')
    }

}

function excluir(){
    listar()

    var num = Number(prompt('\nQual o número do filme a ser excluido? (0 para retornar) '))

    if (num == 0){
        return
    }

    // lê o conteúdo de filmes.txt e atribui para lista
    var lista = localStorage.getItem('filmes.txt')

    //Separa o conteúdo de lista em elementos de vetor a cada ocorrência
    var linhas = lista.split('\n')

    //retorna o número de linha do veror
    var tam = linhas.length

    if(num > tam){
        console.log('Obs.: Número Incorreto')
        return
    }

    //remove do vetor a linha informada
    linhas.splice(num-1, 1)

    //Salva em localStorage o novo conteúdo do vetor linhas(sem a linha excluida)
    //.join('\n') acrcescenta uma quedra de linha ao final de cada elemento
    localStorage.setItem('filmes.txt', linhas.join('\n'))

    console.log('\nOk! Filme removido com sucesso')


}

function estatisticas() {
    titulo('Estatística do Cadastro de Filmes')

    //Se não existe um arquivo com o nome do filmes.txt
    if (! localStorage.getItem('filmes.txt')){
       console.log('OBS:..Não há filmes cadastrados!')
       return
    }

    // lê o conteúdo de filmes.txt e atribui para lista
    var lista = localStorage.getItem('filmes.txt')

    //Separa o conteúdo de lista em elementos de vetor a cada ocorrência
    var linhas = lista.split('\n')

    var tam = linhas.length
    var somaDuracao = 0
    var somaNota = 0

    //Obtém cada linha do vetor linhas e atribui para linha
    for(linha of linhas){
        //separa cada linha em elementos de vetor pela ocorrência do ';'
        var partes = linha.split(';')

        var duracao = Number(partes[2])
        var nota = Number(partes[3])

        somaDuracao = somaDuracao + duracao
        somaNota += nota        
    }

    var mediaDuração = somaDuracao / tam
    var mediaNota = somaNota / tam

    console.log(`\nNúm FIlmes Cadastrados: ${tam}`)
    console.log(`\nDuração Média dos FIlmes Cadastrados: ${mediaDuração.toFixed(1)}`)
    console.log(`\nNota Média dos FIlmes Cadastrados: ${mediaNota.toFixed(1)}`)


}

